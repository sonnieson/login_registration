...
// CHECK IF FIELDS ARE NOT EMPTY
    if(!empty(trim($_POST['username'])) && !empty(trim($_POST['user_email'])) && !empty($_POST['user_password'])) && !empty($_POST['user_phone no.']){
...
// Escape special characters.
        ...
        $user_phone.no =mysqli_real_escape_string($db_connection, htmlspecialchars($_POST['user_phone no.']));
...
// INSER USER INTO THE DATABASE
                $insert_user = mysqli_query($db_connection, "INSERT INTO `users` (username, user_email, user_password,user_phone no.) VALUES ('$username', '$user_email', '$user_hash_password')");
